...
public class Mushroom : EnemyBase
{
    ...
    private Vector2 _fanDirection;  // 固定扇形方向
    ...
    protected override bool CheckAttackHit()
    {
        ...
        // 攻击生效时，立即固定扇形方向并显示（不管玩家是否在扇形内）
        _fanDirection = (_player.position - transform.position).normalized;
        ...
        float angle = Vector2.Angle(_fanDirection, toPlayer);
    ...
    {
        // 安全检查：确保 _fanDirection 有效
        if (_fanDirection.sqrMagnitude < 0.01f)
        {
            _fanDirection = Vector2.right;
        }

        float halfAngle = _fanAngle * 0.5f;
        Vector3 center = transform.position;

        Debug.Log($"[Mushroom Draw] center={center}, fanDir={_fanDirection}, halfAngle={halfAngle}, fanRange={_fanRange}");
        ...
        Vector3 leftDir = Quaternion.Euler(0, 0, halfAngle) * (Vector3)_fanDirection;
        ...
        for (int i = 1; i <= _fanSegments; i++)
        {
            ...
            Vector3 point = center + Quaternion.Euler(0, 0, currentAngle) * (Vector3)_fanDirection * _fanRange;
        ...
        Vector3 rightDir = Quaternion.Euler(0, 0, -halfAngle) * (Vector3)_fanDirection;

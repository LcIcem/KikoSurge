...
public class CameraSpiralEffect : ICameraEffect
{
    ...
    private float ampIntensity; // 当前幅度强度，每帧向零衰减
    private float totalTime; // 效果累计总时间(t)
    private bool isTriggered; // 特效是否触发标识
    private Transform cam; // 相机引用，用于基于相机坐标计算偏移
    ...
    {
        Vector2 osc = MathUtil.GetXYOscPoint(totalTime, ampIntensity * maxAmp, angularSpeed, Mathf.PI / 2);                                                                          
        // 相机局部 XOY 平面的 2D 震荡映射到世界坐标
        return cam.right * osc.x + cam.up * osc.y;
    }
    ...
    {
        ampIntensity = Mathf.Clamp(intensity, 0f, 1f);
        totalTime = 0f;
        isTriggered = true;
    }

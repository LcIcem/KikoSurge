...
/// </summary>
public class AddressablesMgr : MonoSingleton<AddressablesMgr>
{
    private void Log(string msg) => Debug.Log($"[AddressablesMgr] {msg}");
    private void LogError(string msg) => Debug.LogError($"[AddressablesMgr] {msg}");
    ...
    private IEnumerator CheckAndDownload(UnityAction<float> onProgress, UnityAction<bool> onComplete)
    {
        ...
        // [3] 获取需要下载的资源总大小
        var size = Addressables.GetDownloadSizeAsync(update.Result);
        yield return size;
        ...
        // [4] 下载所有更新资源
        var download = Addressables.DownloadDependenciesAsync(update.Result);
    ...
    {
        // 先按地址加载所有资源，再按标签过滤
        var addrHandle = Addressables.LoadAssetsAsync<T>(address, null);
        ...
        {
            Addressables.Release(addrHandle);
            return all;
        }

        // 按标签过滤（需要先获取标签对应的资源，再取交集）
        var labelHandle = Addressables.LoadAssetsAsync<T>(label, null);
